...
func (componentFirmwareVersionL) LoadFirmwareComponentFirmwareSetMaps(ctx context.Context, e boil.ContextExecutor, singular bool, maybeComponentFirmwareVersion interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeComponentFirmwareVersion.(*ComponentFirmwareVersion)
		if !ok {
			object = new(ComponentFirmwareVersion)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeComponentFirmwareVersion)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeComponentFirmwareVersion))
			}
		}
	} else {
		s, ok := maybeComponentFirmwareVersion.(*[]*ComponentFirmwareVersion)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeComponentFirmwareVersion)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeComponentFirmwareVersion))
			}
		}
	}
...
func (o *ComponentFirmwareVersion) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return ComponentFirmwareVersionExists(ctx, exec, o.ID)
}
