...
func testServerToManyTargetServerFirmwareSetValidationFacts(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	b.TargetServerID = a.ID
	c.TargetServerID = a.ID
	...
	check, err := a.TargetServerFirmwareSetValidationFacts().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if v.TargetServerID == b.TargetServerID {
			bFound = true
		}
		if v.TargetServerID == c.TargetServerID {
			cFound = true
		}
	}
	...
	slice := ServerSlice{&a}
	if err = a.L.LoadTargetServerFirmwareSetValidationFacts(ctx, tx, false, (*[]*Server)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TargetServerFirmwareSetValidationFacts); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.TargetServerFirmwareSetValidationFacts = nil
	if err = a.L.LoadTargetServerFirmwareSetValidationFacts(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TargetServerFirmwareSetValidationFacts); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testServerToManyAddOpTargetServerFirmwareSetValidationFacts(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddTargetServerFirmwareSetValidationFacts(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.ID != first.TargetServerID {
			t.Error("foreign key was wrong value", a.ID, first.TargetServerID)
		}
		if a.ID != second.TargetServerID {
			t.Error("foreign key was wrong value", a.ID, second.TargetServerID)
		}

		if first.R.TargetServer != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.TargetServer != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.TargetServerFirmwareSetValidationFacts[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.TargetServerFirmwareSetValidationFacts[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.TargetServerFirmwareSetValidationFacts().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
