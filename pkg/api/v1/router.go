...
func (r *Router) Routes(rg *gin.RouterGroup) {
	...
	{
		// uuid is the server id, mode is 'inband' or 'outofband' -- the data generated by each method can differ
		// significantly, so we keep both sets as separate things to make apples-to-apples comparisons easier.
		srvInventory.GET("/:uuid/:mode", amw.AuthRequired(readScopes("server", "inventory")), r.getInventory)
		srvInventory.PUT("/:uuid/:mode", amw.AuthRequired(updateScopes("server", "inventory")), r.setInventory)
	}
}
